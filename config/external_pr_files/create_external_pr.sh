...
# Check PR and Update Branch if needed
...
if git show parent-repo/$PR_BRANCH:config/external_pr_files/tracked_files.json &>/dev/null; then
    JSON_EXISTS=true
    JSON_CONTENT=$(git show parent-repo/$PR_BRANCH:config/external_pr_files/tracked_files.json 2>/dev/null || echo "")
...
if echo "$CHANGED_FILES" | grep -q "config/external_pr_files/tracked_files.json"; then
    if git show parent-repo/$PR_BRANCH:config/external_pr_files/tracked_files.json > config/external_pr_files/tracked_files.json 2>/dev/null; then
