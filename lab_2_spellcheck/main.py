...
def calculate_frequency_distance(
    ...
    if not frequencies or not all(isinstance(letter, str) for letter in alphabet):
        return None
...
def get_matches(
    ...
    for token_index in range(len(token)):
        ...
        for candidate_index in range(start, end):
            if not candidate_matches[candidate_index] and token[token_index] == candidate[candidate_index]:
...
def count_transpositions(
    ...
    if not all(isinstance(match, bool) for match in token_matches) or not all(isinstance(match, bool) for match in candidate_matches):
        return None
    ...
    for token_index in range(len(token)):
        if token_matches[token_index]:
            ...
            if candidate_index < len(candidate) and token[token_index] != candidate[candidate_index]:
                transpositions += 1
