...
Ron"""
    ...
    for step in range(seq_len):
        ...
        pruned = searcher.prune_sequence_candidates(new_cands)
        if pruned is not None:
            candidates = pruned
        else:
            candidates = dict(list(new_cands.items())[:beam_width])
    ...
    decoded = []
    for i, word_id in enumerate(best_seq):
        word = None
        if hasattr(word_processor, 'get_word'):
            word = word_processor.get_word(word_id)
        elif hasattr(word_processor, '_storage'):
