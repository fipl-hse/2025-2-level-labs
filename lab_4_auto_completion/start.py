...
def main() -> None:
    ...
    with open("../lab_3_generate_by_ngrams/assets/Harry_Potter.txt", "r", encoding="utf=8") as harry_file:
        harry_text = harry_file.read()

    word_processor = TextProcessor("<EoW>")
    try:
        n_gram_size = 15
        beam_width = 3
        seq_len = 100
        ...
        context_before = parts[0].strip().replace("\n", " ")
        context_after = parts[1].replace("\n", " ")
        context_for_generation = context_before[-n_gram_size:]
