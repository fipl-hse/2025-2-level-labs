...
def main() -> None:
    ...
    print(f"Greedy before: {GreedyTextGenerator(model, processor).run(52, 'Harry')}")

    print(f"Beam before: {BeamSearchTextGenerator(model, processor, 3).run('Harry', 52)}")

    encoded_ussr_sentences = processor.encode_sentences(ussr_letters)
    model.update(encoded_ussr_sentences)

    print(f"Greedy after: {GreedyTextGenerator(model, processor).run(52, 'Harry')}")

    print(f"Beam after: {BeamSearchTextGenerator(model, processor, 3).run('Harry', 52)}")
    ...
    print(f"Dynamic after: {dynamic_generator.run(50, 'Ivanov')}")
