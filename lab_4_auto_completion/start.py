...
def main() -> None:
    ...
    if suggestions:
        decoded_suggestions = word_processor.decode(suggestions[0])
        print("For 6: ", decoded_suggestions.replace("<EoS>", "").strip())


    ngram_trie_lm = NGramTrieLanguageModel(hp_encoded_text, n_gram_size)
    ngram_trie_lm.build()
    ...
    ussr_corp_before = (greedy_generator.run(seq_len=30, prompt="Dear"), beamsearcher.run(prompt="Dear", seq_len=30))
    ...
    ussr_corp_after = (greedy_generator.run(seq_len=30, prompt="Dear"), beamsearcher.run(prompt="Dear", seq_len=30))
