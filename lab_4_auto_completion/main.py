...
class TriePrefixNotFoundError(Exception):
    """Exception raised when required prefix is missed from the tree."""
    pass
...
class WordProcessor(TextProcessor):
    ...
    def encode_sentences(self, text: str) -> tuple:
        ...
        for token in tokens:
            if token == self._end_of_sentence_token:
                if sentence_buffer:
                    result_sentences.append(tuple(sentence_buffer) + (0,))
                    sentence_buffer = []
            else:
                self._put(token)
                word_id = self._storage.get(token)
                if word_id is not None:
                    sentence_buffer.append(word_id)

        if sentence_buffer:
            result_sentences.append(tuple(sentence_buffer) + (0,))
        return tuple(result_sentences)
    ...
    def _tokenize(self, text: str) -> tuple[str, ...]:
        ...
        for w in split_text:
            ...
            if clean_word:
                output_tokens.append(clean_word)
                if w and w[-1] in '!?.':
                    output_tokens.append(self._end_of_sentence_token)
...
class PrefixTrie:
    ...
    def suggest(self, prefix: NGramType) -> tuple:
        ...
        """
        try:
            target_node = self.get_prefix(prefix)
        except TriePrefixNotFoundError:
            return ()
        collected_results = []
        nodes_to_process = [(target_node, prefix)]

        while nodes_to_process:
            processing_node, existing_sequence = nodes_to_process.pop()
            if processing_node.has_children():
                for child in processing_node.get_children():
                    name_of_child = child.get_name()
                    if name_of_child is None:
                        continue
                    new_sequence = existing_sequence + (name_of_child,)
                    nodes_to_process.append((child, new_sequence))
            else:
                if existing_sequence != prefix:
                    collected_results.append(existing_sequence)
        collected_results.sort(key=lambda seq: seq[-1], reverse=True)
