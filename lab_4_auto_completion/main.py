    ...
    def encode_sentences(self, text: str) -> tuple:
        ...
        for element in tokenized_text:
            if element == self._end_of_sentence_token:
                if id_list:
                    id_list.append(self._storage[element])
                encoded_sentences.append(tuple(id_list))
                id_list = []
            else:
                self._put(element)
                id_list.append(self._storage[element])
    ...
    def _tokenize(self, text: str) -> tuple[str, ...]:
        ...
        for element in sentences:
            ...
            if words[0] == "":
                words.remove("")
            for word in words:
                word_token = [symbol for symbol in word if symbol.isalpha()]
                word_tokens.append(''.join(word_token))
            if not word_tokens:
                raise EncodingError("Tokenization resulted in empty output")
...
class TrieNode:
    ...
    def get_children(self, item: int | None = None) -> tuple["TrieNode", ...]:
        ...
        if item is None:
            ...
            children = tuple(child_node for child_node in self._children if child_node.get_name() == item)
            return children
...
class PrefixTrie:
    ...
    def suggest(self, prefix: NGramType) -> tuple:
        ...
        return tuple(sorted(all_sequences, reverse = True))
