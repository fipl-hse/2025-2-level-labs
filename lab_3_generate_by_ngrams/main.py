...
class TextProcessor:
    ...
    def _postprocess_decoded_text(self, decoded_corpus: tuple[str, ...]) -> str | None:
        ...
        if not isinstance(decoded_corpus, tuple) or not decoded_corpus:
            return None
        decoded_str = ''.join(decoded_corpus).replace(self._end_of_word_token, ' ')
        decoded_str = ' '.join(decoded_str.split())
        decoded_str = decoded_str.capitalize()
        decoded_str += '.'
        return decoded_str
...
class NGramLanguageModel:
    ...
    def build(self) -> int:  # type: ignore[empty-body]
        ...
        if not n_grams:
            return 1
        counts = {n_gram: 0 for n_gram in n_grams}
        counts.update({n_gram[:-1]: 0 for n_gram in n_grams})
    ...
    def generate_next_token(self, sequence: tuple[int, ...]) -> dict | None:
        ...
        if not isinstance(sequence, tuple) or not sequence:
            return None
        if not len(sequence) >= self._n_gram_size - 1:
            return None
    ...
    def _extract_n_grams(
        ...
        for index in range(len(encoded_corpus) - (self._n_gram_size - 1)):
            n_gram_list.append(encoded_corpus[index:index + self._n_gram_size])
        return tuple(n_gram_list)
...
class GreedyTextGenerator:
    ...
    def run(self, seq_len: int, prompt: str) -> str | None:
        ...
        for _ in range(seq_len):
            ...
            if not next_element:
                break
            next_element = {key: value for key, value in next_element.items()
                            if value == max(next_element.values())}
            best_next_token = max(next_element.keys())
            generated_seq.append(best_next_token)
            context = tuple(generated_seq[-self._model.get_n_gram_size() + 1:])
...
class BeamSearcher:
    ...
    def get_next_token(self, sequence: tuple[int, ...]) -> list[tuple[int, float]] | None:
        ...
        if not next_tokens:
            return []
        token_with_freq = list(next_tokens.items())
        token_with_freq.sort(key=lambda x: x[1], reverse=True)
        beam_width = min(self._beam_width, len(token_with_freq))
        result = token_with_freq[:beam_width]
        return result
    ...
    def continue_sequence(
        ...
        """
        if not (isinstance(sequence, tuple) and
            isinstance(next_tokens, list) and
            isinstance(sequence_candidates, dict)):
            return None
        ...
        for token, freq in next_tokens:
            if not isinstance(token, int) or not isinstance(freq, float):
                return None
            if freq <= 0:
                return None
            new_seq = sequence + (token,)
            new_probability = current_probability - math.log(freq)
            candidates_copy[new_seq] = new_probability
        del candidates_copy[sequence]
...
class BeamSearchTextGenerator:
    ...
    def run(self, prompt: str, seq_len: int) -> str | None:
        ...
        None is returned
        """
        if not isinstance(prompt, str) or not isinstance(seq_len, int) or seq_len <= 0:
            return None
        ...
        if not candidates:
            return None
        return self._text_processor.decode(min(candidates.items(),
                                            key=lambda tuple_dict_el: tuple_dict_el[1])[0])
