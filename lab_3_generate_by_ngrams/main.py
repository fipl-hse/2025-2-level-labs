...
class TextProcessor:
    ...
    def _tokenize(self, text: str) -> tuple[str, ...] | None:
        ...
        if tokens and tokens [-1] == self._end_of_word_token and text.strip()[-1].isalpha():
            tokens.pop()
    ...
    def get_id(self, element: str) -> int | None:
        ...
        if not isinstance(element, str):
            return None
        return self._storage.get(element, None)
    ...
    def get_token(self, element_id: int) -> str | None:
        ...
        if not isinstance(element_id, int):
            return None
        for token, tid in self._storage.items():
            if tid == element_id:
                return token
        return None
    ...
    def _postprocess_decoded_text(self, decoded_corpus: tuple[str, ...]) -> str | None:
        ...
        text= ''.join(' ' if c == self._end_of_word_token else c for c in decoded_corpus)
