...
class TextProcessor:
    ...
    def _tokenize(self, text: str) -> tuple[str, ...] | None:
        ...
        for el in text:
            if el.isspace():
                text=text.replace(el, self._end_of_word_token)
        tokens=[]
        for el in text:
            if el.isalpha():
                tokens.append(el)
            elif el==self._end_of_word_token and tokens[-1]!=self._end_of_word_token:
                tokens.append(el)
        if not tokens:
            return None
        return tuple(tokens)
...
class BeamSearchTextGenerator:
    ...
    def run(self, prompt: str, seq_len: int) -> str | None:
        ...
        for _ in range(seq_len):
            ...
            if not tokens:
                break
            candidates = self.beam_searcher.continue_sequence(sequence, tokens, candidates)
            if not candidates:
                break
